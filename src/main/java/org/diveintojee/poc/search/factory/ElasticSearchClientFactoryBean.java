...
public class ElasticSearchClientFactoryBean extends AbstractFactoryBean<Client> {
     ...
     * Supported values: {dropcreate|merge|useexisting}
     *
            ...
            case dropcreate:
                for (String indexRootName : config.keySet()) {
                    if (!"settings".equalsIgnoreCase(indexRootName)) {
                        ...
                        new DropCreateIndicesStrategy().execute(elasticsearch.admin().indices(), indexRootName, index);
                    }
                }

                break;
            case merge:
                new MergeIndicesStrategy().execute(elasticsearch, configFormat);
            case useexisting:
                for (String indexRootName : config.keySet()) {
                    if (!"settings".equalsIgnoreCase(indexRootName)) {
                        Map<String, Object> index = (Map<String, Object>) config.get(indexRootName);
                        new UseExistingIndicesStrategy().execute(elasticsearch.admin().indices(), indexRootName, index);
                    }
                }
                break;
        }
